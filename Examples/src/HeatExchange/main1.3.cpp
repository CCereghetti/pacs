...
int main(int argc, char** argv)
{
  ...
  //Risolvo Ux=y
  vector<double> x(M);
  x[M]=y[M-1]/alfa[M-1];		//il vettore x sarà il mio vettore soluzione
  for(int i=M-1; i>0; i--)
  	x[i]=(y[i-1]-c[i-1]*x[i+1])/alfa[i-1];
  	

  // Analitic solution

    vector<double> xx(M+1);
     for(int m=0;m <= M;m++)
       xx[m]=Te+(To-Te)*cosh(sqrt(act)*(1-m*h))/cosh(sqrt(act));
	...
	cout<<"Plot on the screen"<<endl;
	    for(int m = 0; m<= M; m++)
       {
	 		...
	 		std::tie(coor[m],sol[m],exact[m])=
	  		std::make_tuple(m*h*L,Te*(1.+x[m]),xx[m]);
       }
   ...
   case 2:  
	 ...
	 // \t writes a tab 
		...
		std::tie(coor[m],sol[m],exact[m])=
	    std::make_tuple(m*h*L,Te*(1.+c[m]),xx[m]);
       }
      // Using temporary files (another nice use of tie)
    	gp<<"plot"<<gp.file1d(std::tie(coor,sol))<<
       "w lp title 'uh',"<< gp.file1d(std::tie(coor,exact))<<
